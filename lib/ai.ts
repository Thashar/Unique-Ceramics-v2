...
/**
 * Zasada tła wariantu „AI”. Kolor celowo odpowiada `sand` (#E8DFD0) z palety
 * sklepu – temu samemu, co przycisk „Wyprzedano”. Sam hex modelowi nie
 * wystarczał: potrafił zejść w ciemniejszy, cieplejszy tan z winietą przy
 * krawędziach. Dlatego dokładamy wartości RGB, punkt odniesienia („ledwie
 * ciemniejsze od papieru”), listę odcieni zakazanych, zakaz gradientu i winiety
 * oraz regułę rozstrzygającą wątpliwość na korzyść jaśniejszego tła.
 * Zmieniając odcień, popraw hex i RGB w obu miejscach.
 */
const BACKGROUND_RULE = `BACKGROUND COLOR IS MANDATORY: a very light, pale, airy warm beige - hex #E8DFD0, RGB (232, 223, 208) - only a touch darker than off-white paper, with very low saturation. Fill the whole backdrop with that single flat tone, identical behind and underneath the product: no gradient, no vignette, no darker corners or edges, no backdrop falling off into shadow. The background must NOT drift into tan, camel, khaki, ochre, taupe, warm brown, dark sand or any deeper beige, and it must not pick up a warm orange cast from the lighting. If in doubt, make the background lighter rather than darker. The only shadow allowed is a soft, subtle contact shadow directly beneath the product.`;
