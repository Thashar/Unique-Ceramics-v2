...
**Testy (`tests/`, vitest):** pokrywają moduły liczące ceny – `product-price.ts`, `quantity-promo.ts`,
`free-shipping.ts`, `discount-code.ts` i `order-summary.ts`. To czyste funkcje bez bazy i bez Reacta, a decydują o kwocie,
...
- **discount-code.ts** – kody rabatowe i **wycena całego zamówienia** (moduł neutralny, bez bazy): `normalizeCode`, `isValidCodeFormat`, `activeCodePercent`, `codeState` oraz **`priceOrder({ items, quantityPromo, code, shipping })`** – jedyne wejście do wyceny, wspólne dla `CheckoutForm`, `CartView` i `/api/checkout`, dzięki czemu klient widzi dokładnie tę kwotę, którą policzy serwer.
  - **Kolejność:** cena bazowa → przecena produktowa (jest już w `item.price`) → rabat ilościowy → kod → wysyłka (`shippingFor`: odbiór osobisty → 0, dalej promocja „Darmowa wysyłka”, dalej stawka metody).
...
- **DishwasherIcon.tsx** – SVG ikona „można myć w zmywarce": **dwa talerze i krople wody** (wzorzec wskazany przez właściciela 24.08.2026 – sklep sprzedaje ceramikę, więc symbol mówi o naczyniach, nie o sprzęcie AGD; **bez kosza/ociekacza**). Pierwszy talerz to dwa współśrodkowe okręgi (obrys i kołnierz), drugi wystaje zza niego w prawo – jego łuk **musi mieć `large-arc = 1`**, inaczej chowa się w obrysie pierwszego. Nad nimi wachlarz dziesięciu kropel: ścieżki zerowej długości z zaokrąglonym końcem, więc ich średnica równa się grubości kreski i cała ikona skaluje się jednym parametrem (wzorzec ma ich ok. dwudziestu – przy 18 px gęstsza siatka zlewa się w plamę). Konwencja lucide (viewBox 24, `currentColor`, kreska 1,5, zaokrąglone końce), żeby stała równo obok `Truck` i `Clock`. Na karcie produktu używana w rozmiarze 18 px pod opisem (blok wysyłki obok używa 14 px). Propsy: `size` (default 20), `strokeWidth`, `className`
