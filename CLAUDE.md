...
### `components/ui/`
- **ProductCard.tsx** – karta produktu (next/image + framer-motion); gdy zdjęcie główne pochodzi z AI, w prawym dolnym rogu kadru stoi `AiImageBadge` (kadr jest opakowany dodatkowym `relative`, bo dymek musi wyjść poza `overflow-hidden` zdjęcia); przyjmuje opcjonalny prop `compact?: boolean` – zmniejsza czcionkę tytułu (`text-lg`→`text-sm`), cenę, kategorię, marginesy i badge'y w widoku kompaktowym
- **AiImageBadge.tsx** – `"use client"`, oznaczenie „✦ AI ⓘ” **w prawym dolnym rogu zdjęcia** wygenerowanego przez model (rozpoznawane po `isAiGeneratedImage`). Siedzi w ciemnej bryle (`bg-espresso/85` + tekst `cream`), więc jest czytelne niezależnie od kolorystyki zdjęcia. Rozmiary `size`: `lg` (galeria produktu), `md` (kafelek listy), `sm` (widok kompaktowy) – ikony skalowane klasami, nie propem `size` lucide'a, żeby dało się je zmieniać responsywnie. `bare` wyłącza własną bryłę, gdy znaczek dzieli ją z licznikiem zdjęć. Dymek z treścią `AI_IMAGE_NOTICE` („Wybrane elementy grafiki…”) pokazuje się po najechaniu kursorem i po kliknięciu na dotyku, zamyka go klik poza znaczkiem lub Escape. Całość jest przyciskiem, a `onClick`/`onPointerDown` zatrzymują zdarzenie – znaczek leży wewnątrz `<Link>` (lista) i wewnątrz kadru z gestami (galeria), więc bez tego kliknięcie otwierałoby produkt albo lupę
