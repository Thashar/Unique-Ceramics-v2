...
- **GalleryEditor.tsx** – `"use client"`, edytor galerii zdjęć przy opisie (zakładki „O mnie" i „Warsztaty" w /admin/ustawienia): wgrywanie wielu plików naraz przez `/api/admin/upload`, kadrowanie każdego zdjęcia (`FocalPointPicker`, proporcje 4/3 – ten sam kadr poziomy co na stronie), zmiana kolejności strzałkami, usuwanie z potwierdzeniem, limit `GALLERY_MAX_IMAGES` (30). Zwraca JSON przez `onChange` (wzorzec jak `WorkshopsOffersEditor`)
