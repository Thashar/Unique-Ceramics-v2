...
**Testy (`tests/`, vitest):** pokrywają moduły liczące ceny – `product-price.ts`, `bundled-shipping.ts`,
`discount-code.ts` i `order-summary.ts`. To czyste funkcje bez bazy i bez Reacta, a decydują o kwocie,
...
- **Product** – `id`, `slug` (unique), `name`, `description`, `price`, `images[]`, `category`, `stock`, `featured`, `active`, `discountPercent` (rabat produktowy w %, 0 = brak; **sumuje się** z promocją „Wielosztuki” – schodzi z ceny bazowej, dopiero potem dochodzi narzut na wysyłkę), `discountStartsAt` / `discountEndsAt` (nullable – okno obowiązywania rabatu w **UTC**; panel ustawia je w **czasie polskim**, `null` = odpowiednio „od razu” i „bezterminowo”)
...
- **Order** – `id`, `userId` (nullable – `null` = zamówienie bez konta lub konto usunięte), `status`, `total`, `shippingCost`, pola adresowe, `paymentMethod` (`transfer`/`stripe`), `paymentStatus` (`pending`/`PAID`/`expired`), `shippingMethod` (`courier`/`parcel_locker`/`pickup`), `parcelLockerCode` (nullable), `trackingNumber` (nullable), `trackingCarrier` (nullable – `dpd`/`dhl`/`inpost`/`poczta`), `paidAt` (nullable – data i godzina wpłaty)
  ...
  - `bundleSurcharge` (nullable) – narzut promocji „Wielosztuki” wliczony w ceny katalogowe **w chwili zakupu** (`null` = promocja wtedy nie obowiązywała). Podsumowanie starego zamówienia odtwarzamy z tego pola, a **nie** z bieżącego ustawienia `bundled_shipping_enabled` – inaczej wyłączenie promocji w panelu zmieniało wygląd zamówień sprzed miesięcy
...
- **DiscountCode** – kod rabatowy: `id`, `code` (unique, wielkimi literami), `percent` (1–90), `active`, `stackable`, `startsAt` / `endsAt` (okno w UTC, panel ustawia je w czasie polskim), `usedCount`, indeks `[active]`. Konfiguracja jak rabat w karcie produktu, plus decyzja o łączeniu: `stackable = true` – kod schodzi dodatkowo z cen po przecenach i promocja „Wielosztuki” działa normalnie; `false` – kod działa **sam** (bez przecen produktów i bez „Wielosztuk”), a `priceOrder` liczy oba warianty i zostawia **tańszy dla klienta** (remis = promocje sklepu, kod zostaje niewykorzystany). **Brak tabeli nie wywraca sklepu** – odczyty są w try/catch, kody są wtedy nieaktywne, a panel pokazuje instrukcję migracji
...
| `bundled_shipping_enabled` | "true"/"false" – **promocja „Wielosztuki”** – wysyłka wliczona w cenę produktu (zakładka Promocje w /admin/ustawienia). Gdy aktywny: katalog pokazuje cenę powiększoną o narzut (wyższy z kosztów kuriera i paczkomatu) z zieloną etykietą „Darmowa wysyłka”, w koszyku rabat dostaje **każda sztuka, także pierwsza** – ceny pozycji mnożymy przez wspólny współczynnik (podział kwotowy `narzut / liczba sztuk` dawał tańszym produktom większy % niż droższym), pozycje pokazują cenę sprzed rabatów przekreśloną, cenę po rabatach i %, a podsumowanie wiersze „Produkty przed rabatem” i „Rabat”. **Pokazany procent obejmuje rabat produktowy i rabat za wielosztuki razem** (patrz `lib/bundled-shipping.ts`), a wiersze rabatu pojawiają się także przy wyłączonej promocji, gdy produkt ma własną przecenę. Reszta z zaokrągleń trafia na ostatnią pozycję, żeby suma zgadzała się co do grosza z kwotą liczoną przez serwer. Pod podsumowaniem stoi warunek promocji („Rabat za wielosztuki naliczamy przy zakupie od 2 sztuk – obejmuje wszystkie produkty w zamówieniu”) – **nie usuwaj go**, to jasność warunków wymagana przy komunikowaniu promocji (**rabat za wielosztuki widać dopiero w koszyku** – katalog i karta produktu pokazują cenę katalogową i zachętę „Uzyskaj rabat”, bo upust zależy od liczby sztuk. Nie dotyczy to **rabatu produktowego**: przecenę pokazuje i karta produktu, i kafelek w katalogu – przekreślona cena, cena po rabacie i procent, w katalogu mniejszą czcionką. Do 24.08.2026 kafelek pokazywał samą obniżoną cenę, więc klient przeglądający listę nie miał jak zauważyć promocji). Suma = ceny produktów + jedna wysyłka, czyli dokładnie tyle, ile liczy `/api/checkout` – **promocja jest warstwą prezentacji, kwot serwerowych nie zmienia**. Wiersz wysyłki mówi tylko „Darmowa wysyłka” **wszędzie, gdzie patrzy klient**: koszyk, wybór metody dostawy, podsumowanie zamówienia, strona potwierdzenia, historia zamówień w koncie, e-mail potwierdzający i pozycje sesji Stripe. **Przy odbiorze osobistym** wiersz nazywa się „Odbiór osobisty” i ma wartość „Bezpłatnie” (nie „Wysyłka: darmowa dostawa”) – w podsumowaniu zamówienia, na potwierdzeniu, w koncie i w e-mailu (tam narzut jest wliczony w ceny pozycji, a reszta z zaokrągleń idzie na jedną sztukę; gdyby suma nie trafiła w kwotę zamówienia, wracamy do rozbicia z osobną wysyłką). Potwierdzenie, konto i karta zamówienia w panelu odtwarzają rozbicie przez **`orderSummary()` z `lib/order-summary.ts`** – z pól zapisanych w zamówieniu (`OrderItem.basePrice`, `Order.bundleSurcharge`), nie z bieżących ustawień sklepu. Powiadomienie dla właściciela zostaje na kwotach z bazy. Zasada: **nie pokazujemy klientowi, że przesyłka jest wliczona w cenę** (decyzja właściciela 22.08.2026) – żadnej kwoty 0 zł, dopisków „z wysyłką” przy pozycjach ani zdań tłumaczących mechanikę. W trakcie promocji próg darmowej wysyłki jest ignorowany (także serwerowo), a wybór **kuriera albo paczkomatu** nie zmienia kwoty – narzut jest dla obu ten sam (siedzi w cenach katalogowych). **Odbiór osobisty zeruje wysyłkę zawsze, także w promocji** – cały narzut wraca wtedy do klienta jako rabat (`chargedSurcharge = 0`, patrz `lib/bundled-shipping.ts`). Do 24.08.2026 promocja pobierała narzut również przy odbiorze, mimo że sklep pisał przy nim „Bezpłatnie” – klient dopłacał 18 zł za wysyłkę, której nie było; nie przywracaj tego zachowania |
...
| `/sklep/[slug]` | ISR 60 s (pre-generated) | Szczegóły produktu (**server component**, JSON-LD Product, pełne Open Graph + Twitter card – zdjęcie z `/api/og/[slug]`; pamiętaj, że `openGraph` ze strony **zastępuje** ten z layoutu, więc `siteName`/`locale`/`type` są powtórzone); `generateStaticParams` pre-generuje wszystkie aktywne produkty przy budowaniu. Galeria: `ProductGallery` (client). Opis renderowany z `whitespace-pre-line` – entery z textarea w panelu mają zostać enterami na stronie. Przy rabacie z terminem pod ceną stoi zielony dopisek „Promocyjna cena obowiązuje do {data}” (ikona `Clock`, czas polski) – bez terminu nie ma go wcale. Informacja „Można myć w zmywarce" (`DishwasherIcon` 18 px, `text-sm`) stoi **tuż pod opisem** – celowo większa niż blok pod przyciskiem koszyka (wysyłka `Truck` i czas realizacji `Clock`, ikony 14 px, `text-xs`) |
| `/koszyk` | ISR 300 s | Koszyk **nigdy nie pokazuje kwoty dostawy i nie dolicza jej do sumy** – koszt zależy od metody wybieranej na `/zamowienie` (w promocji „Wielosztuki” suma koszyka jest już kwotą końcową). Strona serwerowa pobiera ustawienia wysyłki (w tym konfigurację testu) i podaje je propsem do `CartView` (`"use client"`). **Nie pobieraj ich fetchem w przeglądarce** – wtedy po wejściu widać przez moment ceny policzone starą stawką, które po chwili podskakują |
| `/zamowienie` | force-dynamic | Formularz zamówienia (server) → `CheckoutForm` (client). Tu klient wpisuje **kod rabatowy** (pole w podsumowaniu; `/api/discount-code` zwraca procent i zasadę łączenia, kwoty liczy `priceOrder`). Kod niełączony, który wypada gorzej niż promocje sklepu, nie jest stosowany – formularz mówi o tym wprost. **Dostępne bez logowania** – patrz „Zamówienie bez konta (checkout gościa)" |
...
| POST | `/api/checkout` | Zamówienie: walidacja, kwoty serwerowo przez `priceOrder` z `lib/discount-code.ts` (cena pozycji = `discountedPrice(product.price, activeDiscountPercent(product))`, do tego promocja „Wielosztuki” i kod rabatowy – rabaty i ich terminy liczy **serwer**, w chwili składania zamówienia; kod z żądania jest sprawdzany w bazie, nieważny po prostu nie wchodzi), **transakcja stock+order**, e-mail / sesja Stripe. Zapisuje też ślad cenowy: `OrderItem.basePrice` i `Order.bundleSurcharge`.
...
| POST | `/api/discount-code` | Weryfikacja kodu rabatowego wpisanego w zamówieniu (publiczne, rate limit 20/min na IP) → `{ code, percent, stackable }`. Nieznany, wyłączony i wygasły kod dostaje ten sam komunikat 404. **Kwoty i tak liczy jeszcze raz `/api/checkout`** |
...
- **bundled-shipping.ts** – logika promocji „Wielosztuki”: `BUNDLED_SHIPPING_KEY`, `bundleFromSettings` (narzut = wyższy z kosztów wysyłki; zerowy narzut = promocja wyłączona), `bundlePrice` (cena katalogowa) i `bundleSummary` (pozycje z `catalogUnitPrice`, `unitPrice` po rabatach, `discountPercent` i `lineTotal` oraz sumy `catalogTotal`, `discountTotal`, `discountPercent`, `itemsTotal`, `surcharge`, `total`). `BundleConfig.chargedSurcharge` (opcjonalne, domyślnie `= surcharge`) mówi, ile z narzutu klient **realnie płaci** – przy odbiorze osobistym `0`, więc cały narzut wraca jako rabat, a ceną odniesienia zostaje `surcharge` z cen katalogowych. Rabat za wielosztuki jest **proporcjonalny**: `ratio = total / promoTotal` (gdzie `promoTotal` liczy ceny **po** rabacie produktowym) schodzi z ceny każdej pozycji, więc narzut rozkłada się równo, a reszta z zaokrągleń ląduje na ostatniej pozycji.
...
- **discount-code.ts** – kody rabatowe i **wycena całego zamówienia** (moduł neutralny, bez bazy): `normalizeCode`, `isValidCodeFormat`, `activeCodePercent` (okno liczone tą samą funkcją co przeceny produktów), `codeState` oraz `priceOrder({ items, bundle, code, shipping })`. `priceOrder` zwraca wariant (`promo`/`code`), ceny jednostkowe do zapisania w zamówieniu, rozbicie do pokazania klientowi (`display` z `bundleSummary`), `promoDiscount`, `codeDiscount`, koszt wysyłki i kwotę końcową. **Używa go i `CheckoutForm`, i `/api/checkout`** – dzięki temu klient widzi dokładnie tę kwotę, którą policzy serwer. Kod niełączony wyklucza przeceny produktów i „Wielosztuki”, więc liczone są oba warianty i wygrywa tańszy; remis zostawia promocje sklepu
...
- **order-summary.ts** – rozbicie kwot **złożonego** zamówienia, wspólne dla strony potwierdzenia, historii zamówień klienta i karty zamówienia w panelu. `orderSummary({ items, shippingCost, total, shippingMethod, bundleSurcharge, discountCode, discountAmount })` zwraca pozycje, `catalogTotal`, `discountTotal`, `discountPercent`, `codeAmount`/`codeLabel`, `shippingShown` i `shippingLabel` (`pickup`/`bundled`/`free`/`paid`). **Gwarantuje, że kolumna się domyka**: `catalogTotal − discountTotal + shippingShown = total` – rabat jest z tej równości *wyliczany*, a nie sumowany z osobnych źródeł. Kod rabatowy jest **częścią** `discountTotal` i pokazujemy go dopiskiem; osobny wiersz odejmowania zaniżał sumę o jego wartość. Czyta wyłącznie dane zapisane w zamówieniu – dzięki temu przestawienie promocji w panelu nie zmienia wyglądu starych zamówień. Pokryte testami w `tests/pricing.test.ts`
- **product-price.ts** – rabat produktowy (`Product.discountPercent` + okno `discountStartsAt`/`discountEndsAt`): `MAX_DISCOUNT_PERCENT` (90), `normalizeDiscountPercent`, `discountedPrice(price, percent)`, `shownDiscountPercent(before, after)`, `activeDiscountPercent(product, { now?, holdMs? })`, `discountState(product)` (`none`/`scheduled`/`active`/`expired` – opisy w panelu) oraz stałe `DISCOUNT_HOLD_CATALOG_MS` (60 s) i `DISCOUNT_HOLD_HOME_MS` (3600 s). `holdMs` to okno cache strony: rabat kończący się **w trakcie** życia zapisanego HTML-a pokazujemy jako już nieaktywny, żeby nigdy nie reklamować ceny niższej niż ta, którą policzy `/api/checkout` (klient najwyżej zapłaci mniej, niż widział – nigdy więcej). Odwrotny kierunek jest nieszkodliwy: przecena bywa widoczna z opóźnieniem do jednego okna cache. Cena po rabacie jest **ceną bazową** wszędzie dalej – w koszyku, w promocji „Wielosztuki” i w kwotach liczonych przez `/api/checkout` (rabat i promocja się sumują). `shownDiscountPercent` liczy procent z **dwóch pokazywanych kwot**: przy włączonej promocji obie niosą narzut na wysyłkę, więc realna obniżka jest niższa niż nominalny rabat (100 zł −20% przy narzucie 18 zł to 118 → 98, czyli −17%) – liczby na karcie produktu muszą się zgadzać. Moduł neutralny, bez bazy
...
- **cart.tsx** – koszyk jako **store modułowy** (`useSyncExternalStore` + localStorage); hook `useCart()`, bez providera. `CartItem.price` to cena **po** rabacie produktowym (tę kwotę liczy `/api/checkout`), a opcjonalne `CartItem.basePrice` – cena podstawowa sprzed rabatu, wyłącznie do pokazania upustu w koszyku. Starsze wpisy w localStorage nie mają `basePrice` i po prostu pokazują mniejszy rabat.
  - **`useCartPriceSync()`** – wyrównuje ceny koszyka do stanu z serwera (`POST /api/cart/prices`) raz po wejściu na `/koszyk` i `/zamowienie`; zwraca `true`, gdy coś się zmieniło (strona pokazuje wtedy komunikat). Bez tego klient oglądał cenę zamrożoną w chwili dodania produktu, a po wygaśnięciu przeceny płacił inną kwotę, niż widział. Ochrona `holdMs` z `product-price.ts` kończy się na kliknięciu „Dodaj do koszyka” – **to jest jej przedłużenie**. Błąd sieci celowo przechodzi bez śladu (kwoty i tak weryfikuje `/api/checkout`)
...
- **ProductPriceTag.tsx** – cena produktu: rabat produktowy + promocja „Wielosztuki”. Przeceniony produkt pokazuje cenę przekreśloną, cenę po rabacie i procent z `shownDiscountPercent` **we wszystkich rozmiarach** – na karcie produktu (`size="lg"`) pełną czcionką, w katalogu (`sm`/`md`) mniejszą. Bez rabatu jest to **cena katalogowa** (baza + narzut), bez przeliczeń rabatu koszykowego. W katalogu pod ceną zielone dopiski „Darmowa wysyłka” i „Uzyskaj rabat”; na karcie produktu (`size="lg"`) zostaje **sama cena** – tam te informacje pokazuje `ProductBundleNotes`. Komponent jest synchroniczny (nie czyta koszyka), więc cena nie mruga po wczytaniu strony
- **ProductBundleNotes.tsx** – `"use client"`, dwa zielone wiersze w bloku pod przyciskiem koszyka na karcie produktu: „Darmowa wysyłka” (`Truck`) i zachęta do rabatu (`BadgePercent`) – przy pustym koszyku „Dodaj drugą sztukę lub więcej, by otrzymać rabat”, przy niepustym „Dodaj produkt do koszyka, aby uzyskać dodatkowy rabat”. Przy wyłączonej promocji nie renderuje nic (wtedy blok pokazuje zwykły koszt wysyłki)
