...
| POST | `/api/checkout` | Zamówienie: walidacja, kwoty serwerowo (cena pozycji = `discountedPrice(product.price, activeDiscountPercent(product))` – rabat produktowy i jego termin liczy **serwer**, w chwili składania zamówienia), **transakcja stock+order**, e-mail / sesja Stripe. Działa z sesją i bez niej (`userId` = `null` dla gościa); gość musi przesłać `acceptTerms: true`, a przy wysyłce (kurier/paczkomat) wymagany jest telefon. E-mail potwierdzający idzie do klienta przy **obu** metodach płatności (`buildOrderEmail`) |
...
- **bundled-shipping.ts** – logika promocji „Wielosztuki”: `BUNDLED_SHIPPING_KEY`, `bundleFromSettings` (narzut = wyższy z kosztów wysyłki; zerowy narzut = promocja wyłączona), `bundlePrice` (cena katalogowa) i `bundleSummary` (pozycje z `catalogUnitPrice`, `unitPrice` po rabacie, `discountPercent` i `lineTotal` oraz sumy `catalogTotal`, `discountTotal`, `discountPercent`, `itemsTotal`, `surcharge`, `total`). Rabat jest **proporcjonalny**: `ratio = total / catalogTotal` schodzi z ceny każdej pozycji, więc wszystkie mają **ten sam procent** (`discountPercent` liczony raz dla koszyka, nie z zaokrąglonej ceny sztuki), a reszta z zaokrągleń ląduje na ostatniej pozycji. Neutralny moduł bez bazy: używa go katalog, koszyk, checkout i `/api/checkout`. **Nie zmienia kwot zapisywanych w zamówieniu** – `OrderItem.price` to nadal cena bazowa, a `Order.shippingCost` koszt wysyłki
