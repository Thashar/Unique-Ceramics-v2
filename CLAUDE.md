...
| `category_intro_{slug}` | Tekst wstępu na stronie kategorii (np. `category_intro_kubki`). Pusty = tekst układany z nazwy kategorii przez `lib/category-seo.ts`. Edycja w /admin/kategorie |
...
| `/sklep` | dynamic + dane z cache 60 s | Pełny katalog. Pasek kategorii (`CategoryBar`) prowadzi na **strony kategorii**, a stary filtr `?kategoria=…` dostaje **trwałe przekierowanie** (`permanentRedirect`, 308) na nowy adres – nieznana wartość wraca na `/sklep`. Produkty ładuje wspólne `loadCatalog()` z `app/sklep/catalog.ts`. Nagłówek `h1` jest `sr-only` (treść niesie pasek kategorii i siatka). `ProductGridSkeleton` jest **nieużywany** – siatka renderuje się razem ze stroną, bez `<Suspense>` |
| `/sklep/kategoria/[slug]` | ISR 60 s (pre-generated) | **Strona kategorii** – własny adres, tytuł, opis, canonical, widoczny `h1` z nazwą kategorii, tekst wstępu i `CollectionPage` + `ItemList` (do 30 pozycji) + okruszki. `generateStaticParams` pre-generuje wszystkie kategorie, nieznany slug → `notFound()`. Tekst wstępu: własny z panelu (`category_intro_{slug}`), a gdy pusty – układany z nazwy przez `lib/category-seo.ts`. To **jedyne** indeksowalne adresy kategorii; nie wracaj do `?kategoria=` |
| `/sklep/[slug]` | ISR 60 s (pre-generated) | Szczegóły produktu (**server component**, JSON-LD Product, pełne Open Graph + Twitter card – zdjęcie z `/api/og/[slug]`; `openGraph` ze strony **zastępuje** ten z layoutu, więc `siteName`/`locale`/`type` są powtórzone); `generateStaticParams` pre-generuje wszystkie aktywne produkty. Galeria: `ProductGallery` (client) – przycisk z lupką w rogu kadru otwiera `ImageLightbox` (podgląd z powiększeniem). Opis renderowany z `whitespace-pre-line`. Przy rabacie z terminem pod ceną stoi zielony dopisek „Promocyjna cena obowiązuje do {data}” (`Clock`, czas polski). Blok wysyłki pokazuje **„Wysyłka od {najtańsza stawka}”** – klient nie wybrał jeszcze metody, więc podajemy najniższą z kuriera i paczkomatu; słowo „od” znika, gdy obie kosztują tyle samo (byłoby mylące). Pod nim `QuantityPromoNotes` z trwającymi promocjami. Informacja „Można myć w zmywarce" (`DishwasherIcon` 18 px, `text-sm`) stoi **tuż pod opisem** – celowo większa niż blok pod przyciskiem koszyka (ikony 14 px, `text-xs`) |
...
| `/admin/kategorie` | Zarządzanie kategoriami produktów (CRUD + kolejność) oraz **opisy stron kategorii** (`CategoryIntros` – zapis przez `/api/admin/settings` do `category_intro_{slug}`; puste pole = tekst układany z nazwy, widoczny jako podpowiedź) |
...
- **category-seo.ts** – adresy i teksty stron kategorii (moduł neutralny, bez bazy): `categoryPath(slug)` (`/sklep/kategoria/{slug}`), `categoryIntroKey(slug)` (klucz ustawienia z własnym opisem), `categoryTitle`, `categoryDescription` i `categoryIntro`. **Nazwa kategorii zawsze otwiera zdanie, w mianowniku** – wersje wciągające ją w środek zdania („znajdziesz {nazwa}…”) łamały się na „Inne” czy „Zestawy kawowe”, a kategorie dodaje właściciel. Teksty generowane są tylko wypełniaczem: własny opis z panelu jest dla wyszukiwarki wart więcej niż szablon powtórzony na każdej kategorii
