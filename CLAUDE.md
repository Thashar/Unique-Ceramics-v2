...
- **`app/sklep/CustomOrderTile.tsx`** – ostatni kafelek siatki produktów: zaproszenie do zamówień indywidualnych (wspomina też o wysyłce zagranicznej, bo tą drogą przyjmujemy takie zamówienia). Stoi na końcu **każdej strony** katalogu i każdej kategorii – także przy stronicowaniu, więc klient trafia na niego niezależnie od tego, gdzie skończy przeglądanie; przy pustej kategorii pokazuje się zamiast siatki, pod komunikatem „Brak produktów”. `h-full` zrównuje go z wysokością sąsiednich kart. **Zastąpił pływający przycisk `FloatingOrderButton`** (usunięty 28.08.2026 – zasłaniał produkty i dało się go schować na stałe); nie przywracaj go
