...
| `/warsztaty` | ISR 300 s | Wprowadzenie (galeria po prawej), oferty, „Co zawiera warsztat?" jako **lista + galeria** (`workshops_includes_gallery`; nagłówek nad siatką, żeby galeria startowała równo z nim; bez zdjęć lista zwęża się do `max-w-xl` i zostaje wyśrodkowana) oraz FAQ w **dwóch kolumnach** (`md:grid-cols-2`) zakończone kafelkiem z odesłaniem do `/kontakt`. Mapa ikon jest w `app/warsztaty/icons.ts` – zwykły moduł, bo korzysta z niej i strona serwerowa, i kliencki `WorkshopIncludes` |
