...
`similar-products.ts`, `external-sale-validation.ts`, `image-variants.ts`, `storage-cleanup.ts` i `escape-html.ts`.
...
| `/admin/ustawienia` | Ustawienia sklepu (strona główna, o mnie, warsztaty, regulamin, polityka, kontakt, wysyłka, urlop, zam. indywidualne, AI (zdjęcia i opisy), **proponowane**, **zdjęcia**, promocje, płatności) |
...
- **AiImageButtons.tsx** – `"use client"`, przyciski **AI** i **AI+** pod kafelkiem zdjęcia w panelu, wspólne dla `ProductForm` i `ProjectForm` (razem ze stałą `AI_CONFIRM` i typem `AiGenerating`). Zdjęcie rozpoznane przez `isAiGeneratedImage` dostaje w to miejsce plakietkę „Wygenerowane” – przez model puszczamy każde zdjęcie tylko raz. Stan generowania trzyma formularz, komponent tylko go pokazuje
- **ProjectForm.tsx** – formularz projektu portfolio: tytuł, zdjęcia (upload przez `/api/admin/upload`, limit `PROJECT_MAX_IMAGES` z `lib/portfolio-validation.ts`), opis w `RichEditor` (wariant `rich-content-sm` – tak renderuje go strona), kolejność i widoczność. Pod każdym kafelkiem stoją te same przyciski **AI / AI+** co przy produktach (`AiImageButtons` → `/api/admin/ai-image`, wynik dokładany na koniec listy, oryginał zostaje, zapis do bazy dopiero przy zapisaniu projektu)
...
`/api/admin/rotate`, `/api/admin/ai-image`, `/api/admin/ai-text`, `/api/og/**`
