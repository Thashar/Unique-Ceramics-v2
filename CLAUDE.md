...
- **DishwasherIcon.tsx** – SVG ikona „można myć w zmywarce": **talerz z tryskającą na niego wodą**, nie obudowa zmywarki (decyzja właściciela 24.08.2026 – sklep sprzedaje ceramikę, więc symbol ma mówić o naczyniu, nie o sprzęcie AGD). Lucide nie ma takiego symbolu, więc rysunek trzyma się jego konwencji (viewBox 24, `currentColor`, kreska 1,5, zaokrąglone końce), żeby stał równo obok `Truck` i `Clock`. Na karcie produktu używany w rozmiarze 18 px pod opisem (blok wysyłki obok używa 14 px). Propsy: `size` (default 20), `strokeWidth`, `className`
