...
| `/admin/kategorie` | Zarządzanie kategoriami produktów (CRUD + kolejność), **kolekcjami** (`CollectionsManager` – CRUD serii produktów; kolejność strzałkami) oraz **opisy kategorii dla wyszukiwarki** (`CategoryIntros` – zapis przez `/api/admin/settings` do `category_intro_{slug}`; puste pole = opis układany z nazwy, widoczny jako podpowiedź; licznik ostrzega powyżej 160 znaków, bo tyle pokazuje Google) |
