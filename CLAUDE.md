...
- **bundled-shipping.ts** – logika testu „wysyłka w cenie”: `BUNDLED_SHIPPING_KEY`, `bundleFromSettings` (narzut = wyższy z kosztów wysyłki; zerowy narzut = test wyłączony), `bundlePrice` (cena katalogowa), `bundleUnitPrice`, `bundleDiscountPercent` i `bundleSummary` (pozycje po cenach katalogowych + `catalogTotal`, `discountTotal`, `discountPercent`, `itemsTotal`, `surcharge`, `total`; rabat liczony **na cały koszyk**, nie per produkt). Neutralny moduł bez bazy: używa go katalog, koszyk, checkout i `/api/checkout`. **Nie zmienia kwot zapisywanych w zamówieniu** – `OrderItem.price` to nadal cena bazowa, a `Order.shippingCost` koszt wysyłki
