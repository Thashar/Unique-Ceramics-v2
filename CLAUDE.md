...
- **Order** – `id`, `userId` (nullable – `null` = zamówienie bez konta lub konto usunięte), `status`, `total`, `shippingCost`, pola adresowe, `paymentMethod` (`transfer`/`stripe`), `paymentStatus` (`pending`/`PAID`/`expired`), `shippingMethod` (`courier`/`parcel_locker`/`pickup`), `parcelLockerCode` (nullable), `trackingNumber` (nullable), `trackingCarrier` (nullable – `dpd`/`dhl`/`inpost`/`poczta`), `paidAt` (nullable – data i godzina wpłaty)
  - `discountCode` / `discountAmount` (nullable) – ślad po użytym kodzie rabatowym. **Kwota jest już wliczona w ceny pozycji** (`OrderItem.price`), więc wiersz w podsumowaniach jest informacyjny – nie odejmuj go drugi raz
...
- **ProductPriceTag.tsx** – cena produktu: rabat produktowy + promocja „Wielosztuki”. Przeceniony produkt pokazuje na **karcie produktu** (`size="lg"`) cenę przekreśloną, cenę po rabacie i procent z `shownDiscountPercent`; bez rabatu i w katalogu jest to **cena katalogowa po rabacie** (baza − rabat + narzut), bez przeliczeń rabatu koszykowego. W katalogu pod ceną zielone dopiski „Darmowa wysyłka” i „Uzyskaj rabat”; na karcie produktu (`size="lg"`) zostaje **sama cena** – tam te informacje pokazuje `ProductBundleNotes`. Komponent jest synchroniczny (nie czyta koszyka), więc cena nie mruga po wczytaniu strony
