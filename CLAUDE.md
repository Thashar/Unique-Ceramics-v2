...
- **Product** – `id`, `slug` (unique), `name`, `description`, `price`, `images[]`, `category`, `stock`, `featured`, `active`, `discountPercent` (rabat produktowy w %, 0 = brak; **sumuje się** z promocją „Wielosztuki” – schodzi z ceny bazowej, dopiero potem dochodzi narzut na wysyłkę)
...
- **product-price.ts** – rabat produktowy (`Product.discountPercent`): `MAX_DISCOUNT_PERCENT` (90), `normalizeDiscountPercent`, `discountedPrice(price, percent)` i `shownDiscountPercent(before, after)`. Cena po rabacie jest **ceną bazową** wszędzie dalej – w koszyku, w promocji „Wielosztuki” i w kwotach liczonych przez `/api/checkout` (rabat i promocja się sumują). `shownDiscountPercent` liczy procent z **dwóch pokazywanych kwot**: przy włączonej promocji obie niosą narzut na wysyłkę, więc realna obniżka jest niższa niż nominalny rabat (100 zł −20% przy narzucie 18 zł to 118 → 98, czyli −17%) – liczby na karcie produktu muszą się zgadzać. Moduł neutralny, bez bazy
