...
| `home_hero_eyebrow`, `home_hero_title`, `home_hero_text`, `home_hero_cta_primary`, `home_hero_cta_secondary`, `home_hero_scroll` | **Teksty sekcji hero** – napis nad nagłówkiem, nagłówek, opis, etykiety obu przycisków i napis przy strzałce na dole. Edytowalne w /admin/ustawienia (zakładka „Strona główna”). W nagłówku i opisie Enter łamie wiersz (`whitespace-pre-line`), a **puste ustawienie ukrywa dany element** – można zostawić samo zdjęcie. Adresy przycisków są stałe (`/sklep`, `/o-mnie`); domyślne treści siedzą w `lib/home-hero.ts` |
...
| `/sklep/[slug]` | ISR 60 s (pre-generated) | Szczegóły produktu (**server component**, JSON-LD Product, pełne Open Graph + Twitter card – zdjęcie z `/api/og/[slug]`; `openGraph` ze strony **zastępuje** ten z layoutu, więc `siteName`/`locale`/`type` są powtórzone); `generateStaticParams` pre-generuje wszystkie aktywne produkty. Galeria: `ProductGallery` (client) – kliknięcie w zdjęcie otwiera `ImageLightbox` (podgląd z powiększeniem). Opis renderowany z `whitespace-pre-line`. Przy rabacie z terminem pod ceną stoi zielony dopisek „Promocyjna cena obowiązuje do {data}” (`Clock`, czas polski). Blok wysyłki pokazuje **„Wysyłka od {najtańsza stawka}”** – klient nie wybrał jeszcze metody, więc podajemy najniższą z kuriera i paczkomatu; słowo „od” znika, gdy obie kosztują tyle samo (byłoby mylące). Pod nim `QuantityPromoNotes` z trwającymi promocjami. Informacja „Można myć w zmywarce" (`DishwasherIcon` 18 px, `text-sm`) stoi **tuż pod opisem** – celowo większa niż blok pod przyciskiem koszyka (ikony 14 px, `text-xs`) |
...
- **home-hero.ts** – teksty sekcji hero strony głównej: `HOME_HERO_DEFAULT` (domyślne treści – zarazem podpowiedzi w polach panelu), `HOME_HERO_KEYS` i `HOME_HERO_SETTING_KEYS` (klucze do jednego `getSettings`). Neutralny moduł bez bazy – korzysta z niego `lib/settings.ts` (defaulty), strona główna, kliencki `Hero` i `SettingsForm`. Zmieniając domyślny tekst, zmieniaj go **tutaj**, nie w komponencie
...
- **FooterInstagramPanel.tsx** – `"use client"`, animowany panel Instagram; `instagram` jest opcjonalny – bez propsa handle dociągany jest z `/api/public/contacts` (dzięki temu stopka podstron zostaje synchroniczna). Pod przyciskiem Instagrama stoi **ikona Facebooka**: gdy `contact_facebook` jest wypełnione – link do profilu, a dopóki jest puste – wyszarzona, nieklikalna ikona z dopiskiem „Facebook – wkrótce” (profil dopiero powstanie, a link donikąd byłby gorszy niż zapowiedź). Wpisanie adresu w panelu zamienia ją w link samo
...
### `components/home/`
- **Hero.tsx** – pierwszy ekran strony głównej. **Cały tekst przychodzi propsami z ustawień** (`home_hero_*` – patrz `lib/home-hero.ts`); pusty props ukrywa element (nagłówek, opis, każdy z przycisków i napis przy strzałce), a nowe wiersze w nagłówku i opisie łamią tekst przez `whitespace-pre-line`. Domyślne wartości propsów są tylko awaryjne – treść ustawia panel
- **FeaturedProducts.tsx**, **AboutTeaser.tsx**, **WorkshopsTeaser.tsx**
