...
- **Header.tsx** – responsywna nawigacja, ikona koszyka, menu mobilne. Napis „Unique Ceramics / Ręcznie tworzone z sercem” obok logo jest **czystą bielą (`text-white`) w obu stanach headera, bez przezroczystości i bez zmiany koloru na hoverze** – ma się zgadzać z logo (`brightness-0 invert`). Wcześniej nad jasnymi sekcjami szedł `text-cream`, co obok białego logo czytało się jako kawowy odcień, a `/70` na podtytule dawało wrażenie szarej obwódki; **nie przywracaj tam `text-cream`**; gdy `menuOpen` header zawsze przyjmuje `bg-espresso` (niezależnie od sekcji hero).
  ...
  - **Motyw liczy `IntersectionObserver`, nie pomiar przy zdarzeniach.** Wcześniej stan brał się z `getBoundingClientRect()` przy `scroll`/`resize` plus kilka kontrolnych przeliczeń po starcie – taki pomiar jest migawką, więc jedna nietrafiona (układ jeszcze się układał, trwało programowe przewijanie) zostawała **na stałe**, bo `scroll` odpala się tylko przy zmianie pozycji. Objaw: ciemny header nad widocznym hero, „od czasu do czasu”, nie do powtórzenia na żądanie (naprawione 28.08.2026). Obserwator przelicza się sam – przy pierwszej obserwacji, po doładowaniu zdjęć i fontów, po zmianie rozmiaru okna, po powrocie z bfcache i przy każdym przekroczeniu progu (`IO_THRESHOLDS` zagęszczone wokół `VISIBLE_RATIO` = 0,3). **Nie wracaj do liczenia rect-ów przy `scroll`.** Sekcja bez wysokości (jeszcze nieułożona) nie może przestawić headera na ciemny, a brak sekcji w DOM tylko ponawia próbę obserwacji (~1,5 s). Zdarzenia `scroll`/`resize` zostały wyłącznie dla zanikania headera w stopce, które musi jechać klatka po klatce.
