...
- **DishwasherIcon.tsx** – SVG ikona „można myć w zmywarce": **dwa nachodzące talerze i krople wody** (sklep sprzedaje ceramikę, więc symbol mówi o naczyniach, nie o sprzęcie AGD; bez kosza/ociekacza). ⚠️ **Talerze nie mają kołnierza (okręgu w okręgu).** Wersja ze współśrodkowymi okręgami zostawiała między kreskami 0,7 jednostki prześwitu, więc zlewały się w ciemną tarczę i ikona odstawała czernią od `Truck` i `Clock` obok – zmierzona waga optyczna 26,3 wobec 24,9 dla `Clock`. Dwa **osobne, nachodzące** talerze dają 22,9 przy tej samej lucide'owej kresce 1,5; **nie wracaj do współśrodkowych okręgów**. Łuk tylnego talerza musi mieć `large-arc = 1`, inaczej chowa się w przednim. Osiem kropel jako ścieżki zerowej długości z zaokrąglonym końcem – średnica równa się grubości kreski, więc cała ikona skaluje się jednym parametrem. Na karcie produktu w rozmiarze 18 px pod opisem (blok wysyłki obok używa 14 px), w `text-clay` jak sąsiednie ikony. Propsy: `size` (default 20), `strokeWidth`, `className`
