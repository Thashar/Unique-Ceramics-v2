...
export async function GET() {
  ...
  try {
    ...
    return NextResponse.json({ available: true, collections });
  } catch (e) {
    // Migracja tabeli jest ręczna – brak tabeli nie może wywrócić panelu,
    // zamiast listy pokazujemy instrukcję (jak przy kodach rabatowych)
    console.error("GET /api/admin/collections:", e);
    return NextResponse.json({ available: false, collections: [] });
  }
}
