...
export function workshopsMetadata(locale: Locale): Metadata {
  const m = t(locale).meta;
  return pageMetadata({
    title: m.workshopsTitle,
    description: m.workshopsDescription,
    path: "/warsztaty",
    // Podgląd linku = zdjęcie hero warsztatów (z panelu)
    image: ogImage(OG_PAGE_IMAGE.workshops, m.workshopsOgTitle),
    locale,
  });
}
...
/**
 * Strona warsztatów – wspólna dla `/warsztaty` i `/en/warsztaty`. Wprowadzenie,
 * oferty, lista „Co zawiera warsztat?” i FAQ po angielsku idą z kluczy
 * `en_workshops_*` (panel → Warsztaty → EN); bez nich zostaje polska treść.
 */
export default async function WorkshopsPage({ locale = "pl" }: { locale?: Locale }) {
  const d = t(locale);
  ...
  const faq = parseJson<WorkshopFaq>(localizedSetting(locale, "workshops_faq", s, en));

  const BASE = "https://uniqueceramics.pl";
  const pageUrl = `${BASE}${localePath(locale, "/warsztaty")}`;
  ...
  return (
    <>
      ...
      <BreadcrumbSchema locale={locale} items={[{ name: d.nav.workshops, path: "/warsztaty" }]} />
      <Header locale={locale} />
      <main className="flex-1">
        {/* Hero */}
        {heroImage ? (
          <div className="relative overflow-hidden" style={{ height: `${heroHeight}vh` }}>
            <Image src={heroImage} alt={d.workshops.heroAlt} fill priority className="object-cover" style={{ objectPosition: heroPos }} sizes="100vw" />
                      ...
                      <div className="space-y-3 mb-8">
                        {w.duration && (
                          <div className="flex justify-between text-sm">
                            <span className="text-charcoal/80">{d.workshops.duration}</span>
                            <span className="text-espresso font-medium">{w.duration}</span>
                          </div>
                        )}
                        {w.maxPeople && (
                          <div className="flex justify-between text-sm">
                            <span className="text-charcoal/80">{d.workshops.participants}</span>
                        ...
                        {w.priceLabel && (
                          <div className="flex justify-between text-sm border-t border-sand pt-3">
                            <span className="text-charcoal/80">{d.workshops.price}</span>
                            <span className="font-serif text-xl text-espresso">{w.priceLabel}</span>
                          </div>
                        )}
                      </div>
                      <Link href={localePath(locale, "/kontakt")} className="block text-center bg-clay hover:bg-terracotta hover:text-espresso text-warm-white text-xs tracking-widest uppercase py-4 transition-colors rounded-md">
                        {d.workshops.book}
                      </Link>
                    </div>
                  </div>
            ...
            <div className={`mx-auto ${hasIncludesGallery ? "max-w-6xl" : "max-w-xl"}`}>
              {hasIncludesGallery ? (
                <>
                  ...
                  <ClayRule className="mb-7" />
                  <WorkshopIncludes includes={includes} images={includesGallery} title={d.workshops.includes} />
                </>
              ) : (
                <>
                  <h2 className="font-serif text-[2rem] md:text-4xl text-espresso mb-5 text-center">{d.workshops.includes}</h2>
        ...
        {faq.length > 0 && (
          <div className="bg-mist py-20 px-6 lg:px-10">
            <div className="max-w-5xl mx-auto">
              <h2 className="font-serif text-[2rem] md:text-4xl text-espresso mb-5 text-center">{d.workshops.faq}</h2>
              <ClayRule align="center" className="max-w-[220px] mx-auto mb-12" />
              ...
              <div className="mt-12 bg-espresso p-7 flex flex-wrap items-center justify-between gap-5 rounded-2xl">
                <div>
                  <p className="font-serif text-xl text-cream mb-1">{d.workshops.faqMissing}</p>
                  <p className="text-sm text-sand/90">{d.workshops.faqWrite}</p>
                </div>
                <Link
